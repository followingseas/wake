...
- **🔦 대화 내용 검색** — `⌘⇧F`로 모든 프로젝트의 대화 본문을 찾습니다. 기억나는 단어 하나로 어느 세션의 어느 메시지였는지 짚어내고, 결과를 클릭하면 그 메시지로 바로 이동합니다.
...
| 세션 검색 | `⌘F` — 세션 제목·첫 메시지 대상 |
| 대화 내용 검색 | `⌘⇧F` — 모든 세션의 대화 본문 대상. 스니펫 클릭 시 해당 메시지로 이동 |
...
- 원본 파일에 가하는 유일한 쓰기 동작은 사용자가 명시적으로 요청한 삭제이며, 이 또한 파일을 휴지통으로 옮기는 방식으로 동작합니다.
